...
构建侧车：`pnpm run build:clawhub`、`pnpm run build:knowledge`、`pnpm run build:outcomes`（仅编译，不触发子目录单独 install）。
...
| `/api/oneearning/outcomes/*` | [outcomes](./outcomes/) 侧车（项目成果抽取） | 启动时 `getPort()` 动态分配 |
