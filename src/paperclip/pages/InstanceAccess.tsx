...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("paperclip.crumbs.accessShort") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("paperclip.toasts.instanceAccess.companyUpdated"), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("paperclip.toasts.instanceAccess.roleUpdated"), tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("paperclip.instanceAccessPage.loadingUsers")}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("paperclip.instanceAccessPage.error403")
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : t("paperclip.instanceAccessPage.errorLoad");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("paperclip.instanceAccessPage.title")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">{t("paperclip.instanceAccessPage.intro")}</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("paperclip.instanceAccessPage.searchUsersLabel")}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t("paperclip.instanceAccessPage.searchPlaceholder")}
            />
          </label>
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("paperclip.instanceAccessPage.activeMemberships", { count: user.activeCompanyMembershipCount })}
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("paperclip.instanceAccessPage.selectUserHint")}</div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">{t("paperclip.instanceAccessPage.loadingAccess")}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("paperclip.instanceAccessPage.loadAccessFailed")}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin
                    ? t("paperclip.instanceAccessPage.removeInstanceAdmin")
                    : t("paperclip.instanceAccessPage.promoteInstanceAdmin")}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("paperclip.instanceAccessPage.companyAccessTitle")}</h2>
                  <p className="text-sm text-muted-foreground">{t("paperclip.instanceAccessPage.companyAccessIntro")}</p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending
                      ? t("paperclip.instanceAccessPage.savingCompanyAccess")
                      : t("paperclip.instanceAccessPage.saveCompanyAccess")}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("paperclip.instanceAccessPage.currentMemberships")}</h2>
                <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {(membership.membershipRole
                            ? t(`paperclip.companyAccessPage.membershipRole_${membership.membershipRole}`, {
                                defaultValue: membership.membershipRole,
                              })
                            : t("paperclip.companyAccessPage.unset"))}
                          {" • "}
                          {t(`paperclip.companyAccessPage.memberStatus_${membership.status}`, {
