...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("paperclip.crumbs.profileSettings") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("paperclip.accountMenu.defaultDisplayName");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("paperclip.forms.profile.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("paperclip.forms.profile.avatarCompanyRequired"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("paperclip.forms.profile.uploadFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("paperclip.forms.profile.removeAvatarFailed"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("paperclip.profileSettingsPage.loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("paperclip.forms.profile.loadFailed")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("paperclip.accountMenu.defaultDisplayName");
  ...
  const uploadHint = selectedCompany
    ? t("paperclip.profileSettingsPage.uploadHintWithCompany", { companyName: selectedCompany.name })
    : t("paperclip.profileSettingsPage.uploadHintNoCompany");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("paperclip.profileSettingsPage.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("paperclip.profileSettingsPage.subtitle")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("paperclip.profileSettingsPage.changePhoto") : t("paperclip.profileSettingsPage.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {t("paperclip.profileSettingsPage.remove")}
                    </Button>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("paperclip.profileSettingsPage.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("paperclip.profileSettingsPage.avatarHint")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("paperclip.profileSettingsPage.displayNameLabel")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("paperclip.profileSettingsPage.placeholderName")}
            />
            <p className="text-xs text-muted-foreground">
              {t("paperclip.profileSettingsPage.displayNameHint")}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("paperclip.profileSettingsPage.emailLabel")}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("paperclip.profileSettingsPage.emailHint")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("paperclip.profileSettingsPage.saving") : t("paperclip.profileSettingsPage.save")}
            </Button>
          </div>
        </form>
      </section>
