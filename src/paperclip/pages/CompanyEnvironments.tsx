...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    <span className="inline-flex items-center gap-1 text-green-700 dark:text-green-400">
      ...
      {t("paperclip.companyEnvironmentsPage.yes")}
    </span>
  ) : (
    <span className="text-muted-foreground">{t("paperclip.companyEnvironmentsPage.no")}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("paperclip.crumbs.companyEnvironments") },
    ]);
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId
          ? t("paperclip.toasts.companyEnvironments.environmentUpdated")
          : t("paperclip.toasts.companyEnvironments.environmentCreated"),
        body: t("paperclip.toasts.companyEnvironments.environmentReady", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("paperclip.toasts.companyEnvironments.saveEnvironmentFailed"),
        body: error instanceof Error ? error.message : t("paperclip.toasts.companyEnvironments.saveEnvironmentFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok
          ? t("paperclip.toasts.companyEnvironments.environmentProbePassed")
          : t("paperclip.toasts.companyEnvironments.environmentProbeFailed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      pushToast({
        title: t("paperclip.toasts.companyEnvironments.environmentProbeFailed"),
        body: error instanceof Error ? error.message : t("paperclip.toasts.companyEnvironments.probeFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok
          ? t("paperclip.toasts.companyEnvironments.draftProbePassed")
          : t("paperclip.toasts.companyEnvironments.draftProbeFailed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("paperclip.toasts.companyEnvironments.draftProbeFailed"),
        body: error instanceof Error ? error.message : t("paperclip.toasts.companyEnvironments.probeFailedBody"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("paperclip.companyEnvironmentsPage.selectCompany")}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("paperclip.companyEnvironmentsPage.title")}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("paperclip.companyEnvironmentsPage.disabledBody")}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("paperclip.companyEnvironmentsPage.intro")}
        </p>
      </div>

      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("paperclip.companyEnvironmentsPage.supportMatrixNote")}
        </div>

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("paperclip.companyEnvironmentsPage.supportMatrixCaption")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("paperclip.companyEnvironmentsPage.colAdapter")}</th>
                <th className="px-3 py-2 font-medium">{t("paperclip.companyEnvironmentsPage.colLocal")}</th>
                <th className="px-3 py-2 font-medium">{t("paperclip.companyEnvironmentsPage.colSsh")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("paperclip.companyEnvironmentsPage.colSandbox")}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("paperclip.companyEnvironmentsPage.emptyList")}</div>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string" ? environment.config.host : t("paperclip.companyEnvironmentsPage.sshHostFallback")} ·{" "}
                          {typeof environment.config.username === "string" ? environment.config.username : t("paperclip.companyEnvironmentsPage.sshUserFallback")}
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {(() => {
                            ...
                            return t("paperclip.companyEnvironmentsPage.sandboxProviderLine", {
                              name: displayName,
                        ...
                        <div className="text-xs text-muted-foreground">{t("paperclip.companyEnvironmentsPage.runsOnHost")}</div>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {environment.driver !== "local" ? (
                        ...
                        >
                          {environmentProbeMutation.isPending
                            ? t("paperclip.companyEnvironmentsPage.testing")
                            : environment.driver === "ssh"
                              ? t("paperclip.companyEnvironmentsPage.testConnection")
                              : t("paperclip.companyEnvironmentsPage.testProvider")}
                        </Button>
                      ) : null}
                      <Button
                        size="sm"
                      ...
                      >
                        {isEditing ? t("paperclip.companyEnvironmentsPage.editing") : t("paperclip.companyEnvironmentsPage.edit")}
                      </Button>
                    </div>
                  </div>
                  {probe ? (
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId ? t("paperclip.companyEnvironmentsPage.editEnvironment") : t("paperclip.companyEnvironmentsPage.addEnvironment")}
          </div>
          <div className="space-y-3">
            <Field label={t("paperclip.companyEnvironmentsPage.fieldName")} hint={t("paperclip.companyEnvironmentsPage.fieldNameHint")}>
              <input
            ...
            <Field label={t("paperclip.companyEnvironmentsPage.fieldDescription")} hint={t("paperclip.companyEnvironmentsPage.fieldDescriptionHint")}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("paperclip.companyEnvironmentsPage.fieldDriver")} hint={t("paperclip.companyEnvironmentsPage.fieldDriverHint")}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                <option value="ssh">{t("paperclip.companyEnvironmentsPage.driverSsh")}</option>
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("paperclip.companyEnvironmentsPage.driverSandbox")}</option>
                ) : null}
                <option value="local">{t("paperclip.companyEnvironmentsPage.driverLocal")}</option>
              </select>
            </Field>

            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("paperclip.companyEnvironmentsPage.fieldHost")} hint={t("paperclip.companyEnvironmentsPage.fieldHostHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("paperclip.companyEnvironmentsPage.fieldPort")} hint={t("paperclip.companyEnvironmentsPage.fieldPortHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                    min={1}
                ...
                <Field label={t("paperclip.companyEnvironmentsPage.fieldUsername")} hint={t("paperclip.companyEnvironmentsPage.fieldUsernameHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("paperclip.companyEnvironmentsPage.fieldRemotePath")} hint={t("paperclip.companyEnvironmentsPage.fieldRemotePathHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("paperclip.companyEnvironmentsPage.fieldPrivateKey")} hint={t("paperclip.companyEnvironmentsPage.fieldPrivateKeyHint")}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    ...
                    >
                      <option value="">{t("paperclip.companyEnvironmentsPage.noSavedSecret")}</option>
                      {(secrets ?? []).map((secret) => (
                ...
                <Field label={t("paperclip.companyEnvironmentsPage.fieldKnownHosts")} hint={t("paperclip.companyEnvironmentsPage.fieldKnownHostsHint")}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("paperclip.companyEnvironmentsPage.strictHostKeyChecking")}
                    hint={t("paperclip.companyEnvironmentsPage.strictHostKeyCheckingHint")}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("paperclip.companyEnvironmentsPage.fieldProvider")} hint={t("paperclip.companyEnvironmentsPage.fieldProviderHint")}>
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("paperclip.companyEnvironmentsPage.providerNoExtraConfig")}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ? t("paperclip.companyEnvironmentsPage.saving")
                    : t("paperclip.companyEnvironmentsPage.creating")
                  : editingEnvironmentId
                    ? t("paperclip.companyEnvironmentsPage.saveEnvironment")
                    : t("paperclip.companyEnvironmentsPage.createEnvironment")}
              </Button>
              {editingEnvironmentId ? (
                <Button
                  size="sm"
                ...
                >
                  {t("paperclip.companyEnvironmentsPage.cancel")}
                </Button>
              ) : null}
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending ? t("paperclip.companyEnvironmentsPage.testing") : t("paperclip.companyEnvironmentsPage.testDraft")}
                </Button>
              ) : null}
              {environmentMutation.isError ? (
                <span className="text-xs text-destructive">
                  {environmentMutation.error instanceof Error
                    ...
                    : t("paperclip.companyEnvironmentsPage.failedSaveEnvironment")}
                </span>
              ) : null}
