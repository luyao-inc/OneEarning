...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("paperclip.crumbs.instanceHeartbeats") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("paperclip.instanceHeartbeatsPage.updateFailed"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("paperclip.instanceHeartbeatsPage.disableAllFailed"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("paperclip.instanceHeartbeatsPage.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("paperclip.instanceHeartbeatsPage.loadFailed")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("paperclip.instanceHeartbeatsPage.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">{t("paperclip.instanceHeartbeatsPage.intro")}</p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>
          <span className="font-semibold text-foreground">{activeCount}</span> {t("paperclip.instanceHeartbeatsPage.statsActive")}
        </span>
        <span>
          <span className="font-semibold text-foreground">{disabledCount}</span> {t("paperclip.instanceHeartbeatsPage.statsDisabled")}
        </span>
        <span>
          <span className="font-semibold text-foreground">{grouped.length}</span>{" "}
          {t("paperclip.instanceHeartbeatsPage.statsCompany", { count: grouped.length })}
        </span>
        {anyEnabled && (
          <Button
            variant="destructive"
            ...
            onClick={() => {
              if (!window.confirm(t("paperclip.instanceHeartbeatsPage.disableAllConfirm", { count: enabledCount }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("paperclip.instanceHeartbeatsPage.disabling") : t("paperclip.instanceHeartbeatsPage.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState icon={Clock3} message={t("paperclip.instanceHeartbeatsPage.empty")} />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("paperclip.instanceHeartbeatsPage.badgeOn") : t("paperclip.instanceHeartbeatsPage.badgeOff")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("paperclip.instanceHeartbeatsPage.never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("paperclip.instanceHeartbeatsPage.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving
                              ? t("paperclip.instanceHeartbeatsPage.saving")
                              : agent.heartbeatEnabled
                                ? t("paperclip.instanceHeartbeatsPage.disableTimer")
                                : t("paperclip.instanceHeartbeatsPage.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
