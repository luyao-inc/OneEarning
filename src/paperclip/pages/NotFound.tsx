...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("paperclip.crumbs.notFound") }]);
  }, [setBreadcrumbs, t]);
  ...
  const title =
    scope === "invalid_company_prefix"
      ? t("paperclip.notFoundPage.titleInvalidCompany")
      : t("paperclip.notFoundPage.titlePage");
  const description =
    scope === "invalid_company_prefix"
      ? t("paperclip.notFoundPage.descInvalidCompany", { prefix: normalizedPrefix ?? "unknown" })
      : t("paperclip.notFoundPage.descGlobal");
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("paperclip.notFoundPage.requestedPath")} <code className="font-mono">{currentPath}</code>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Button asChild>
            <Link to={dashboardHref}>
              ...
              {t("paperclip.notFoundPage.openDashboard")}
            </Link>
          </Button>
          <Button variant="outline" asChild>
            <Link to="/">{t("paperclip.notFoundPage.goHome")}</Link>
          </Button>
        </div>
      </div>
    </div>
