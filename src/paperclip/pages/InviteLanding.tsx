...
function mapInviteAuthFeedback(
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message:
        "That email and password did not match an existing OneEarning account. Check both fields, or create an account first if you are new here.",
    };
  }
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || "this OneEarning company";
          ...
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-[0.24em] text-zinc-500">
                  You&apos;ve been invited to join OneEarning
                </p>
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo" ? "Set up OneEarning" : `Join ${companyDisplayName}`}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ...
                    : requiresHumanAccount
                      ? "Create your OneEarning account first. If you already have one, switch to sign in and continue the invite with the same email."
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? "OneEarning board"}</div>
              </div>
              <div className="border border-zinc-800 p-3">
            ...
            {showsAgentForm ? (
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? `Start with a OneEarning account. After that, you'll come right back here to accept the invite for ${companyDisplayName}.`
                      : "Use the OneEarning account that already matches this invite. If you do not have one yet, switch back to create account."}
                  </p>
                </div>
                ...
                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                    ? "Already signed up before? Use the existing-account option instead so the invite lands on the right OneEarning user."
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ...
                      : `This will ${
                          invite.inviteType === "bootstrap_ceo" ? "finish setting up OneEarning" : `submit or complete your join request for ${companyDisplayName}`
                        }.`}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
