...
export function Activity() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("paperclip.crumbs.activity") }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={History} message={t("paperclip.activity.selectCompany")} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Select value={filter} onValueChange={setFilter}>
          <SelectTrigger className="w-[140px] h-8 text-xs">
            <SelectValue placeholder={t("paperclip.activity.filterPlaceholder")} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t("paperclip.activity.allTypes")}</SelectItem>
            {entityTypes.map((type) => (
              <SelectItem key={type} value={type}>
                {t(`paperclip.activity.entityTypes.${type}`, {
      ...
      {filtered && filtered.length === 0 && (
        <EmptyState icon={History} message={t("paperclip.activity.empty")} />
      )}
