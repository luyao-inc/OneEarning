  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          <span className="text-muted-foreground">{t("paperclip.projectsPage.statusLabel")}</span>
          <div className="mt-1">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("paperclip.projectsPage.targetDateLabel")}</span>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t("paperclip.projectsPage.changeColorAria")}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={t("paperclip.projectsPage.selectColorAria", { color })}
              />
            ))}
          </div>
        </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("paperclip.crumbs.project");
      if (archived) {
        pushToast({ title: t("paperclip.toasts.projectDetail.archived", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("paperclip.toasts.projectDetail.unarchived", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("paperclip.toasts.projectDetail.archiveFailed") : t("paperclip.toasts.projectDetail.unarchiveFailed"),
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t("paperclip.crumbs.project") },
    ]);
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("paperclip.crumbs.project"),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
    ...
    <div className="space-y-6">
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.pauseReason === "budget" ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-red-200">
              ...
              {t("paperclip.projectsPage.budgetPausedBadge")}
            </div>
          ) : null}
        </div>
      </div>
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">{t("paperclip.projectsPage.loadingWorkspaces")}</p>
        )
      ) : null}
