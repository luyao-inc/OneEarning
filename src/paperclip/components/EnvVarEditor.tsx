...
export function EnvVarEditor({
  ...
  async function sealRow(index: number) {
    ...
    const name = window.prompt(t("paperclip.envVarEditor.secretNamePromptTitle"), suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(error instanceof Error ? error.message : t("paperclip.envVarEditor.createSecretFailed"));
    }
  }
  ...
  return (
    <div className="space-y-1.5">
        ...
        return (
          <div key={index} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("paperclip.envVarEditor.keyPlaceholder")}
              value={row.key}
            ...
            >
              <option value="plain">{t("paperclip.envVarEditor.typePlain")}</option>
              <option value="secret">{t("paperclip.envVarEditor.typeSecret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("paperclip.envVarEditor.selectSecret")}</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("paperclip.envVarEditor.titleSealCreate")}
                >
                  {t("paperclip.envVarEditor.newSecret")}
                </button>
              </>
            ) : (
              <>
                <input
                  ...
                  placeholder={t("paperclip.envVarEditor.valuePlaceholder")}
                ...
                <button
                  ...
                  onClick={() => sealRow(index)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("paperclip.envVarEditor.titleSealPlain")}
                >
                  {t("paperclip.envVarEditor.seal")}
                </button>
              </>
            )}
            {!isTrailing ? (
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("paperclip.envVarEditor.runtimeInjectNote")}
      </p>
    </div>
  );
}
