...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("paperclip.sidebar.agentRowResume") : t("paperclip.sidebar.agentRowPause");
  const pauseResumeDisabled = disabled || agent.status === "pending_approval" || isBudgetPaused;
  const pauseResumeDisabledLabel = disabled
    ? t("paperclip.sidebar.agentRowUpdating")
    : isBudgetPaused
      ? t("paperclip.sidebar.agentRowBudgetPaused")
      : pauseResumeLabel;
        ...
        {(agent.pauseReason === "budget" || runCount > 0) && (
          <span className="ml-auto flex items-center gap-1.5 shrink-0">
            {agent.pauseReason === "budget" ? (
              <BudgetSidebarMarker title={t("paperclip.sidebar.agentRowBudgetMarkerTitle")} />
            ) : null}
            {runCount > 0 ? (
            ...
            {runCount > 0 ? (
              <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
                {t("paperclip.sidebar.agentLiveShort", { count: runCount })}
              </span>
            ) : null}
          </span>
        )}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("paperclip.sidebar.agentRowOpenActionsAria", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-44">
          <DropdownMenuItem asChild>
            ...
            >
              ...
              <span>{t("paperclip.sidebar.agentRowEdit")}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("paperclip.sidebar.agentRowBudgetPauseHint") : undefined}
          >
...
export function SidebarAgents() {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? t("paperclip.sidebar.toastAgentPaused") : t("paperclip.sidebar.toastAgentResumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("paperclip.sidebar.toastPauseFailed") : t("paperclip.sidebar.toastResumeFailed"),
        ...
        <div className="flex items-center px-3 py-1.5">
          <CollapsibleTrigger className="flex items-center gap-1 flex-1 min-w-0">
            ...
            <span className="text-[10px] font-medium uppercase tracking-widest font-mono text-muted-foreground/60">
              {t("paperclip.sidebar.sectionAgents")}
            </span>
          </CollapsibleTrigger>
          <button
            onClick={(e) => {
            ...
            aria-label={t("paperclip.sidebar.ariaNewAgent")}
          >
