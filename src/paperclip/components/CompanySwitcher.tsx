  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">
              {selectedCompany?.name ?? t("paperclip.workspace.switcherSelectCompany")}
            </span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-[220px]">
        <DropdownMenuLabel>{t("paperclip.workspace.switcherCompaniesHeading")}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        {sidebarCompanies.length === 0 && (
          <DropdownMenuItem disabled>{t("paperclip.workspace.noCompanies")}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild>
          <Link to="/company/settings" className="no-underline text-inherit">
            <Settings className="h-4 w-4 mr-2" />
            {t("paperclip.workspace.companySettings")}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/companies" className="no-underline text-inherit">
            <Plus className="h-4 w-4 mr-2" />
            {t("paperclip.workspace.manageCompanies")}
          </Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
