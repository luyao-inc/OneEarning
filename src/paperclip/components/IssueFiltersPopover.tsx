      ...
      <PopoverTrigger asChild>
        <Button
          ...
          title={
            iconOnly
              ? (activeFilterCount > 0
                ? t("paperclip.issuesList.filterButtonWithCount", { count: activeFilterCount })
                : t("paperclip.issuesList.filterButton"))
              : undefined
          }
        >
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("paperclip.issuesList.filtersHeading")}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
              ...
              >
                {t("paperclip.issuesList.clearFilters")}
              </button>
            ) : null}
          </div>

          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.quickFilters")}</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionStatus")}</span>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionPriority")}</span>
                <div className="space-y-0.5">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionAssignee")}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("paperclip.issuesList.filterNoAssignee")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("paperclip.issuesList.filterAssigneeMe")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionCreator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("paperclip.issuesList.removeCreatorAria", { name: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("paperclip.issuesList.searchCreatorsPlaceholder")}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("paperclip.issuesList.noCreatorsMatch")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionProject")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionLabels")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionWorkspace")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("paperclip.issuesList.filterSectionVisibility")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("paperclip.issuesList.filterLiveOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("paperclip.issuesList.filterHideRoutineRuns")}</span>
                  </label>
                ) : null}
              </div>
            </div>
